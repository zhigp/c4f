...
// Noise Factor:                zeta        [0.00, 1.00)
// Noise Applied to Inputs:     1 ± Rand[0,1)
// Noise Decreases with the Growth of Cycles

// Learning Rate:               beta        [0.01, 1.00]
// Momentum:                    alpha       [0.00, 1.00]

// Weight Change:               beta * input * errors_bp + alpha * wchange(last cycle)
...
public:
    ...
    virtual void UpdateWeights(float beta, float alpha) {};  // beta: learning rate
                                                             // alpha: momentum
...
protected:
    ...
    float   *deltawt_accum;     // accumulated weight changes in current cycle
    float   *deltawt_prev;      // total weight changes in last cycle
};
...
public:
    ...
    virtual void CalculateOuts();
    virtual void CalculateErrors();
    virtual void UpdateWeights(float beta, float alpha);
...
private:
    void    AcquireTrainingParms();
    void    AcquireTestingParms();
    void    AcquireNetworkSz();
    ...
    void    UpdateNoise();
    void    UpdateMomentum();
    void    UpdateStatParms();

    void    PrintCycleStats();               // Training Mode, statistics / cycle
    void    WriteCycleStats();
    void    PrintTotalStats();               // Training Mode, statistics after training finished
    void    WriteTotalStats();
...
private:
    ...
    float       zeta_ini;           // Training Mode: initial noise factor  [0.00, 1.00)
    float       zeta;               // Training Mode: noise decreases with the growth of cycles
    float       beta;               // Training Mode: learning rate     [0.01, 1.00]
    float       alpha;              // Training Mode: momentum          [0.00, 1.00]
